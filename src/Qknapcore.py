    ...
    def main(self):
        ...
        while True:
            ...
            if args.tsp_solver == "ortools":
                ...
                #    → LKH: dist_matrixベース（ローカルindex）
                #    → Amplify: 同じdist_matrixでグローバルIDの経路を出す
                from TSP import TSP
                ...
                for cluster_id in range(len(clusters_coordx)):
                    ...
                    # グローバルIDの対応表（0: depot, それ以外: 元インスタンスの顧客ID）
                    ...
                    lkh_result = solve_tsp_lkh(
                        ...
                        runs=100,
                        seed=0,              # 0 or None → 実行ごとに異なるシード
                        lkh_bin=args.lkh_bin,
                    )

                    # LKHが返すのは「dist_matrixのインデックス」＝ローカルID
                    lkh_route_local = lkh_result["route"]           # 例: [0, 3, 5, 6, 2, 7, 1, 4]
                    # これを city_list にマップしてグローバルIDに変換
                    lkh_route_global = [int(city_list[i]) for i in lkh_route_local]
                    ...
                    tsp_routes.append({
                        ...
                        # LKH 側（ローカルとグローバル両方残す）
                        "lkh_route_local":        lkh_route_local,          # dist_matrix上のインデックス
                        "lkh_route":              lkh_route_global,         # CVRP全体の顧客ID
                        ...
                        # Amplify 側
                        "amplify_route":          to_native(amp_route),
                        "amplify_total_distance": amp_tot,
                        "amplify_solver":         "amplify",
                    })

                    # イテレーション全体のサマリとしては、とりあえず LKH の距離を採用
...
- Amplify（Fixstars）でクラスタリング間の QA ベースの移動を行う。
- 各イテレーションごとにクラスタ内 TSP を解く（OR-Tools / LKH / Amplify）。
- 改善がなくても、そのイテレーションで一度 TSP を解いてから終了。
- ループ終了後に、最終状態に対してもう一度「全クラスタで TSP」を解き、
  final_routes.json として保存する。
- NumPy をすべてネイティブ型に変換して JSON に保存する。
"""
...
# プロジェクトルートを import path に追加（scripts/ から実行しても動くように）
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
...
from src.vrpfactory import vrpfactory
from src.knap_divpro import knap_dippro
from src.tsp_ortools_solver import solve_tsp_ortools  # OR-Tools 版
from src.tsp_lkh_solver import solve_tsp_lkh          # LKH 版
from TSP import TSP                                   # Amplify 版 TSP
...
class Core:
    ...
    def main(self):
        ap = argparse.ArgumentParser(
            description="Iterative QA-based CVRP optimizer (Amplify + TSP solvers)"
        )
        ap.add_argument("-j",   help="Path to before_data.json",             type=str, required=True)
        ap.add_argument("-sp",  help="Base output directory (e.g. ./out)",   type=str, required=True)
        ap.add_argument("--t",  help="Annealing time (ms)",                  type=int, default=3000)
        ...
        ap.add_argument(
            ...
            choices=["ortools", "lkh", "amplify"],
            default="ortools",
            help="TSP solver: 'ortools' / 'lkh' / 'amplify'",
        )
        ap.add_argument(
            "--tsp_time_limit_ms",
            type=int,
            default=2000,
            help="TSP solver time limit per cluster (ms)",
        )
        ap.add_argument(
            "--lkh_bin",
            type=str,
            default=os.environ.get("LKH_BIN"),
            help="Path to LKH binary (default: $LKH_BIN)",
        )
        ap.add_argument("--eps", help="(unused, compat)",                    type=float, default=1e-3)
        args = ap.parse_args()
        ...
        while True:
            ...
            for idx, current_cluster_index in enumerate(perms):
                ...
                print(
                    f"[swap {idx}] move={did_move} | qa={qa_ms:.1f}ms | "
                    f"total={block_ms:.1f}ms | before={sum_before:.3f} | after={sum_after:.3f}"
                )

            # swap のログを保存（改善の有無に関わらず）
            swap_log_path = save_dir / f"iteration_{iteration}_swap_timings.json"
            with open(swap_log_path, "w") as f:
                json.dump(swap_time_log, f, indent=2, default=to_native)
            print(f"🕒 Saved swap details: {swap_log_path}")

            # ---- クラスタ内 TSP を全クラスタに対して実行 ----
            total_distance = 0.0
            tsp_routes: List[Dict[str, Any]] = []
            ...
            if args.tsp_solver == "ortools":
                ...
                # LKH と Amplify を同じ距離行列で両方解き、比較できるようにする
                for cluster_id in range(len(clusters_coordx)):
                    coordx = [depo_x] + clusters_coordx[cluster_id]
                    coordy = [depo_y] + clusters_coordy[cluster_id]
                    cluster_distance = vrpfactory.make_cluster_distance_matrix(coordx, coordy)
                    ...
                    lkh_route_local = lkh_result["route"]
                    lkh_route_global = [int(city_list[i]) for i in lkh_route_local]
                    lkh_tot = float(lkh_result["total_distance"])
                    ...
                    tsp_routes.append({
                        ...
                        # LKH 側（ローカルとグローバル両方残す）
                        "lkh_route_local":        lkh_route_local,
                        "lkh_route":              lkh_route_global,
                        "lkh_total_distance":     lkh_tot,
                        "lkh_solver":             "lkh",
                        "lkh_status":             lkh_result.get("solver_status", ""),
                        "lkh_solve_time_ms":      lkh_result.get("solve_time_ms", None),
                    ...
                    # イテレーション全体のサマリとしては LKH の距離を採用
                    total_distance += lkh_tot

            else:
                # Amplify(TSP) のみでクラスタ内 TSP を解く
                for cluster_id in range(len(clusters_coordx)):
                    coordx = [depo_x] + clusters_coordx[cluster_id]
                    coordy = [depo_y] + clusters_coordy[cluster_id]
                    cluster_demand = [0] + cluster_demands[cluster_id]
